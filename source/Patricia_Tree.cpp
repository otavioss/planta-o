...
Patricia_Tree::Patricia_Tree(int junk)
{   
...
bool Patricia_Tree::getBitAtPos(int value, int position)
{ 
    return (value & (1 << position));
}

int Patricia_Tree::getFirstDiffBitPos(int value1, int value2)
{
    int position = 13;  // max number of bits allowed for input 
                        // 9999 = 14 bits (1-14) -> offset so (0-13)

    while ((getBitAtPos(value1, position) == getBitAtPos(value2, position)) && position >= 0)
    {
...
bool Patricia_Tree::insert(int value, ostream &ofile)
{
    ...
    if (!m_root)
    {
        m_root = insert(value, returnedBitCheck); // returns pointer to root
        insertion = true;
    }
    else insertion = insert(m_root, value, returnedBitCheck); // returns true/false

    if (insertion == true)
    {
        cout << "Successful insertion of " << value << " with node bit test " << returnedBitCheck << endl;
        ofile << "Successful insertion of " << value << " with node bit test " << returnedBitCheck << endl;
        return true;
    }
    else
    {
        cout << "Illegal attempt to insert duplicate value: " << value << endl;
        ofile << "Illegal attempt to insert duplicate value: " << value << endl;
        return false;
    }
}   
        
bool Patricia_Tree::lookup(int value, ostream &ofile)
{
    bool search = false;
    if (m_root) search = lookup(m_root, value);

    if (search == true)
    {
        cout << "Value found: " << value << endl; 
        ofile << "Value found: " << value << endl;
        return true;
    }
    else
    {
        cout << "Value not found: " << value << endl;
        ofile << "Value not found: " << value << endl;
        return false;
    }
}   
    
bool Patricia_Tree::remove(int value, ostream &ofile)
{
    bool removed = false;
    if (m_root) removed = remove(m_root, value);

    if (removed == true)
    {
        cout << "Successful deletion of " << value << endl;
        ofile << "Successful deletion of " << value << endl;
        return true;
    }
    else
    {
        cout << "Illegal attempt to delete a value not present: " << value << endl;
        ofile << "Illegal attempt to delete a value not present: " << value << endl;
        return false;
    }
...
Patricia_Tree::Node *Patricia_Tree::insert(int value, int &returnedBitCheck)
{
    m_root = new Node();
    int diffBit = getFirstDiffBitPos(value, m_dummy->m_value);
...
bool Patricia_Tree::insert(Node *root, int value, int &returnedBitCheck)
{
    int prevBitTest = 0;
    int currBitTest = -1;
                                                                ...
                                                                // bit check AND the root is not at the dummy node
    {
        ...
        int bitTest = getBitAtPos(value, root->m_bit_check);    // check value's bit at the current Node's bit check position
    ...
    int diffBit = getFirstDiffBitPos(value, root->m_value);     // full key compare. If -1, value is a duplicate value and not to
    ...
    else    // there is a higher order bit which differs between the two
    {
        ...
        if (diffBit > predecessor->m_bit_check) // INSERT AS NEW ROOT OR A TWEENER
        {
            ...
            int bitTest = getBitAtPos(newNode->m_value, newNode->m_bit_check);  // get the new node's own bit check
        ...
        {
            int bitTest = getBitAtPos(newNode->m_value, newNode->m_bit_check);  // (value, diffBit)
...
bool Patricia_Tree::lookup(Node *root, int value)
{          
    int prevBitTest = 0;
    int currBitTest = -1;
    ...
    int diffBit = getFirstDiffBitPos(value, root->m_value); 
...
bool Patricia_Tree::remove(Node *root, int value)
{       
    int prevBitTest = 0;
    int currBitTest = -1;
    stack<Node *> ptr_stack; // stack to push/pop pointers of visited nodes
    ...
    int diffBit = getFirstDiffBitPos(value, root->m_value); // full key compare. 
...
void Patricia_Tree::inOrderPrint(ostream &ofile)
{
    if (!m_root)
    {
        cout << "Patricia tree is empty" << endl;
        ofile << "Patricia tree is empty" << endl;
    }
    else 
    {
        inOrderPrint(m_root, 15, ofile);
        cout << endl;
        ofile << endl;
    }
}

void Patricia_Tree::inOrderPrint(Node *root, int previous_bit_check, ostream &ofile)
{
    if (root->m_bit_check >= previous_bit_check)
    {
        cout << ", " << root->m_value << " " << root->m_bit_check;
        ofile << ", " << root->m_value << " " << root->m_bit_check;
        return;
    }
    ...
    inOrderPrint(root->m_left, root->m_bit_check, ofile);
    inOrderPrint(root, root->m_bit_check, ofile);
    inOrderPrint(root->m_right, root->m_bit_check, ofile);
}
